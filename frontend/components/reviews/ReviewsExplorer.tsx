...
export function ReviewForm({ collegeShortName, onDone }: { collegeShortName?: string; onDone?: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    addReview(review);
    setForm({ college: collegeShortName ?? "", course: "", year: "", rating: 4, title: "", body: "", name: "", role: "Student" });
    onDone?.();
  };
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div className="space-y-1.5">
        ...
        <Select id="rv-role" value={form.role} onChange={(e) => setForm((f) => ({ ...f, role: e.target.value }))}>
